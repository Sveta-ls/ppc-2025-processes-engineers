...
bool ZyazevaSVecDotProduct::PreProcessingImpl() {
  ...
  std::cout << "Process " << world_rank << ": PreProcessing started, vectors size = " << input[0].size() << std::endl;
  ...
  if (world_rank == 0) {
    ...
    std::cout << "Process 0: total_elements = " << total_elements << ", delta = " << delta
...
bool ZyazevaSVecDotProduct::RunImpl() {
  ...
  if (world_rank == 0) {
    ...
    std::cout << "Process 0: copied data, first elements: " << local_input1_[0] << ", " << local_input2_[0]
              << std::endl;
    ...
    for (int proc = 1; proc < world_size; proc++) {
      ...
      std::cout << "Process 0: sent " << proc_count << " elements to process " << proc << " starting from offset "
                << offset << std::endl;
      offset += proc_count;
    }
  } else {
    ...
    std::cout << "Process " << world_rank << ": received data, first elements: " << local_input1_[0] << ", "
              << local_input2_[0] << std::endl;
  }
  ...
  std::cout << "Process " << world_rank << ": local_result = " << local_result << ", calculated from " << local_size_
            << " elements" << std::endl;
