...
### Размер векторов \(10^6\)

| Версия выполнения | Время (секунд) | Количество процессов|
|:-----------------|-------------:|-----------------:|
| SEQ pipeline | 0.001024 | 1 |
| MPI pipeline | 0.000306 | 4 | 
| SEQ task_run | 0.000373 | 1 | 
| MPI task_run | 0.009481 | 4 |

MPI медленнее чем SEQ в task_run, но быстрее в pipeline.

### Размер векторов \(10^7\)

| Версия выполнения | Время (секунд) | Количество процессов|
|:-----------------|-------------:|-----------------:|
| SEQ pipeline | 0.005042 | 1 |
| MPI pipeline | 0.004436 | 4 | 
| SEQ task_run | 0.005417 | 1 | 
| MPI task_run | 0.001976 | 4 | 

MPI быстрее SEQ и в task_run и в pipeline.
**Вывод:** при увеличении размерности векторов MPI-версия показывает ускорение примерно в *2.3–2.9 раза*
...
3. При увеличении объёмов данных MPI-версия показывает ускорение.
